...
<div class="filter-box ${showFilterBox}" id="filter-box">
	...
	<%--Court type --%>
	<c:if test="${!empty judgmentCriteriaForm.courtCriteria.courtType}">
		<p><spring:message code="judgmentSearch.formField.courtType" />:</p>
		<div class="filter-item" id="filter-court-type" data-assigned-field="radio-court-${fn:toLowerCase(judgmentCriteriaForm.courtCriteria.courtType)}">
			<div data-tooltip-text="<spring:message code='judgmentSearch.filterBox.removeFilter' />"><saos:enum value="${judgmentCriteriaForm.courtCriteria.courtType}" /></div>
		</div>
	</c:if>
