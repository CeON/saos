...
public class CourtsRepresentationVerifier {
    ...
    public static void verifyBasicFields(ResultActions actions, String pathPrefix) throws Exception{
        actions
                ...
                .andExpect(jsonPath(pathPrefix+".divisions.[0].href").value(endsWith(SINGLE_DIVISIONS_PATH + "/" + FieldsDefinition.JC.DIVISION_ID)))
                .andExpect(jsonPath(pathPrefix+".divisions.[0].name").value(FieldsDefinition.JC.DIVISION_NAME))

                .andExpect(jsonPath(pathPrefix+".divisions.[1].href").value(endsWith(SINGLE_DIVISIONS_PATH + "/" + FieldsDefinition.JC.SECOND_DIVISION_ID)))
